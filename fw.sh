...
if [ "$OLD_DCE" == "yes" ]; then
	...
	ln -s ipc.mak ${BUILD_DIR}/ipc/ipc-bios.mak
	ln -s ipc.bld ${BUILD_DIR}/ipc/ipc-bios.bld
...
make -C ${BASE_ROOT} TARGET=${TARGET} PLATFORM=${PLATFORM} HW_TYPE=${HW_TYPE} PROFILE=${PROFILE} TRACELEVEL=${TRACELEVEL} all
